...
- **Resumable benchmark harness** — Bun-native YAML matrices select pinned corpus fixtures, declare local/hosted and warm/cold state, interleave providers deterministically, score reviewed references without publishing transcript text, capture Darwin peak RSS, and require both explicit hosted opt-in and cleanup evidence for evaluation-owned model downloads.
